<?php /** 
* Generated by
* 
*      _____ _          __  __      _     _
*     / ____| |        / _|/ _|    | |   | |
*    | (___ | | ____ _| |_| |_ ___ | | __| | ___ _ __
*     \___ \| |/ / _` |  _|  _/ _ \| |/ _` |/ _ \ '__|
*     ____) |   < (_| | | | || (_) | | (_| |  __/ |
*    |_____/|_|\_\__,_|_| |_| \___/|_|\__,_|\___|_|
*
* The code generator that works in many programming languages
*
*			https://www.skaffolder.com
*
*
* You can generate the code from the command-line
*       https://npmjs.com/package/skaffolder-cli
*
*       npm install -g skaffodler-cli
*
*   *   *   *   *   *   *   *   *   *   *   *   *   *   *   *
*
* To remove this comment please upgrade your plan here: 
*      https://app.skaffolder.com/#!/upgrade
*
* Or get up to 70% discount sharing your unique link:
*       https://beta.skaffolder.com/#!/register?friend=5d1e567d0e8b622e6162666b
*
* You will get 10% discount for each one of your friends
* 
*/
?>
<?php
	require_once './db/dbappDocente_dbManager.php';
	
/*
 * SCHEMA DB Materia
 * 
	{
		creditos: {
			type: 'Integer'
		},
		id_profesor: {
			type: 'Integer'
		},
		nombre: {
			type: 'String'
		},
		//RELAZIONI
		
		
		//RELAZIONI ESTERNE
		
		
	}
 * 
 */


//CRUD METHODS


//CRUD - CREATE


$app->post('/materia',	function () use ($app){

	$body = json_decode($app->request()->getBody());
	
	$params = array (
		'creditos'	=> isset($body->creditos)?$body->creditos:'',
		'id_profesor'	=> isset($body->id_profesor)?$body->id_profesor:'',
		'nombre'	=> isset($body->nombre)?$body->nombre:'',
			);

	$obj = makeQuery("INSERT INTO materia (_id, creditos, id_profesor, nombre )  VALUES ( null, :creditos, :id_profesor, :nombre   )", $params, false);

	
	echo json_encode($body);
	
});
	
//CRUD - REMOVE

$app->delete('/materia/:id',	function ($id) use ($app){
	
	$params = array (
		'id'	=> $id,
	);

	makeQuery("DELETE FROM materia WHERE _id = :id LIMIT 1", $params);

});
	
//CRUD - GET ONE
	
$app->get('/materia/:id',	function ($id) use ($app){
	$params = array (
		'id'	=> $id,
	);
	
	$obj = makeQuery("SELECT * FROM materia WHERE _id = :id LIMIT 1", $params, false);
	
	
	
	echo json_encode($obj);
	
});
	
	
//CRUD - GET LIST

$app->get('/materia',	function () use ($app){
	makeQuery("SELECT * FROM materia");
});


//CRUD - EDIT

$app->post('/materia/:id',	function ($id) use ($app){

	$body = json_decode($app->request()->getBody());
	
	$params = array (
		'id'	=> $id,
		'creditos'	    => isset($body->creditos)?$body->creditos:'',
		'id_profesor'	    => isset($body->id_profesor)?$body->id_profesor:'',
		'nombre'	    => isset($body->nombre)?$body->nombre:''	);

	$obj = makeQuery("UPDATE materia SET  creditos = :creditos,  id_profesor = :id_profesor,  nombre = :nombre   WHERE _id = :id LIMIT 1", $params, false);

	
	echo json_encode($body);
    	
});


/*
 * CUSTOM SERVICES
 *
 *	These services will be overwritten and implemented in  Custom.js
 */

			
?>